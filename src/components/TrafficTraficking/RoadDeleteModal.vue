...
async function handleDelete() {
  try {
    ...
    await axiosClient.delete(`/api/traffic-tracking/soft-delete/${roadId}`, {
      headers: {
  ...
  <teleport to="body">
    <div v-if="show" class="modal-backdrop fixed inset-0 flex items-center justify-center z-50 "
      ...
      <div @click.stop
        ...
        <!-- Form content -->
         <div v-if="isDeleting" class="p-4 flex flex-col items-center">
            <div class="h-5 w-5 border-b-2 border-white rounded-full animate-spin"></div>
            <p class="text-gray-300 mt-2">Deleting Road</p>
         </div>
