<template>
  <div v-if="activeRoad"
    ...
    <div class="flex flex-col gap-4">
      ...
      <div class="flex">
        <!-- Inbound/Outbound controls with shared structure -->
        <template v-for="direction in ['inbound', 'outbound']" :key="direction">
          <div class="w-1/2">
            <div class="flex gap-4 items-center">
              <h1>{{ direction.charAt(0).toUpperCase() + direction.slice(1) }}</h1>
              <div class="flex items-center cursor-pointer relative" @click.stop="toggleDropdown(direction)">
                ...
                <div v-if="activeDropdown === direction"
                  ...
                  <div v-for="(label, color) in trafficLabels" :key="color"
                    class="p-2 cursor-pointer flex items-center gap-2"
                    @click.stop="changeTraffic(activeRoad.properties.id, direction, color)">
...
defineProps({ activeRoad: Object, colorMap: Object });
const emit = defineEmits(["changeTrafficLevel", "closeEditModal"]);

// State management
const activeDropdown = ref(null);
const trafficLabels = { 'green': 'Light', 'yellow': 'Moderate', 'red': 'Heavy' };
...
const changeTraffic = (roadId, direction, color) => {
